...
import { getAllAttendanceHistory, getUserdatafromserver } from "../../service";
import { RingLoader } from "react-spinners";
import { Search, Filter, RefreshCw, ChevronLeft, ChevronRight, History, Users } from "lucide-react";
...
export default function AdminHistory() {
  ...
  useEffect(() => {
    fetchAdminHistory();
  }, [search, selectedUserId, startDate, endDate, statusFilter, mealFilter, sortOrder]);
    ...
    <div className="space-y-6 text-black dark:text-white my-2">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Complete attendance and tiffin records for all mess members
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-4 shadow-xs">
          ...
          <h3 className="text-2xl font-black text-black dark:text-white mt-1">{summary.totalRecords}</h3>
        </div>

        <div className="bg-green-50 dark:bg-green-950/40 border border-green-200 dark:border-green-800/60 rounded-2xl p-4">
          <p className="text-xs font-semibold text-green-700 dark:text-green-400 uppercase tracking-wider">Total Present</p>
        ...
        <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800/60 rounded-2xl p-4">
          <p className="text-xs font-semibold text-red-700 dark:text-red-400 uppercase tracking-wider">Total Absent</p>
        ...
        <div className="bg-orange-50 dark:bg-orange-950/40 border border-orange-200 dark:border-orange-800/60 rounded-2xl p-4">
          <p className="text-xs font-semibold text-orange-700 dark:text-orange-400 uppercase tracking-wider">Total Lunch</p>
        ...
        <div className="bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-800/60 rounded-2xl p-4">
          <p className="text-xs font-semibold text-purple-700 dark:text-purple-400 uppercase tracking-wider">Total Dinner</p>
        ...
        <div className="bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800/60 rounded-2xl p-4 col-span-2 sm:col-span-1">
          <p className="text-xs font-semibold text-blue-700 dark:text-blue-400 uppercase tracking-wider">Extra Tiffins</p>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800 p-4 sm:p-5 shadow-xs space-y-4">
        ...
        {(search || selectedUserId !== "all" || startDate || endDate || statusFilter !== "all" || mealFilter !== "all" || sortOrder !== "newest") && (
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
        {loading ? (
          ...
          <div className="p-12 text-center text-gray-500 dark:text-gray-400">
            ...
            <p className="text-xs text-gray-400">Try refining your search terms or filter selections.</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                  {paginatedHistory.map((item) => {
                    const uName = item.userName || item.userId?.Name || "User";
                    const uContact = item.userId?.Mobile ? String(item.userId.Mobile) : (item.userId?.Email || "-");
